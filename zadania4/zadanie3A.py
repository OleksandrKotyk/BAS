...
toSend = "zad15odpA;ver;{};srcip;{};dstip;{};type;{}"

version = str(int("0x" + withoutSpace[0], 16))
sourceIP = ("".join([str(int("0x" + withoutSpace[i: i + 2], 16)) + "." for i in range(24, 31, 2)]))[:-1]
destinationIP = ("".join([str(int("0x" + withoutSpace[i: i + 2], 16)) + "." for i in range(32, 39, 2)]))[:-1]
protocol = str(int("0x" + withoutSpace[18:20], 16))

toSend = toSend.format(version, sourceIP, destinationIP, protocol)
